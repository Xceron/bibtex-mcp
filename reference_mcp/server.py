...
def create_server():
    ...
    mcp = FastMCP(
        ...
        instructions="""
        Academic reference search server providing BibTeX citations from multiple sources.
        
        This MCP server searches academic literature databases (DBLP, Semantic Scholar, arXiv, OpenAlex)
        and returns properly formatted BibTeX citations. It provides three tools:
        
        1. search_reference: Full-featured search returning complete BibTeX citations and metadata
        2. search: Lightweight search for browsing (returns only id, title, snippet)
        3. fetch: Retrieve full documents by ID from previous search results
        
        The server automatically deduplicates results across databases and ranks by relevance.
    ...
    async def _search_reference_impl(
        ...
        try:
            ...
            # Format response as dict (not JSON string)
            result = {
